        ...
        async with aiohttp.ClientSession() as session:
            for attempt in range(max_attempts):
                ...
                try:
                    ...
                    logger.info(
                        f"Waiting for vLLM server to be ready. Current check time: {attempt * health_check_interval} "
                        f"seconds. Max timeout: {server_init_timeout} seconds"
                    )
    ...
    async def start(self) -> str:
        ...
        if not started:
            ...
            if self.process.is_alive():
                logger.error(
                    f"vLLM server failed to start within {server_init_timeout} seconds (timeout waiting for event). "
                    "Terminating process."
                )
...
async def start_server_main():
    ...
    try:
        ...
        # Conversation test: send one or two chat completion requests
        served_name_list = server.args.get("served_model_name")
        model_to_call = served_name_list[0] if served_name_list else server.args["model"]

        messages = [
            {"role": "system", "content": "You are a helpful assistant."},
            {"role": "user", "content": "Introduce yourself in one concise sentence."},
        ]
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(server.base_url, json=payload, timeout=60) as resp:
                resp.raise_for_status()
                data = await resp.json()
                assistant_reply = data["choices"][0]["message"]["content"]
                logger.info(f"Assistant: {assistant_reply}")

            # Second turn (multi-turn test)
            messages.append({"role": "assistant", "content": assistant_reply})
            messages.append({"role": "user", "content": "Now summarize your capabilities in three bullet points."})
            payload["messages"] = messages
            async with session.post(server.base_url, json=payload, timeout=60) as resp2:
                resp2.raise_for_status()
                data2 = await resp2.json()
                assistant_reply2 = data2["choices"][0]["message"]["content"]
                logger.info(f"Assistant (2nd turn): {assistant_reply2}")

        # Keep server running for a while for additional manual testing
